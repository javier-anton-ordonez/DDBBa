...
CREATE EVENT IF NOT EXISTS evt_rotar_particiones_posicion
ON SCHEDULE EVERY 1 YEAR
STARTS '2027-01-01 02:30:00'
DO
BEGIN
    DECLARE v_hist BIGINT;
    DECLARE v_ant  BIGINT;
    DECLARE v_act  BIGINT;

    -- límites dinámicos
    SET v_hist = UNIX_TIMESTAMP(DATE_FORMAT(NOW() - INTERVAL 2 YEAR, '%Y-01-01'));
    SET v_ant  = UNIX_TIMESTAMP(DATE_FORMAT(NOW() - INTERVAL 1 YEAR, '%Y-01-01'));
    SET v_act  = UNIX_TIMESTAMP(DATE_FORMAT(NOW(), '%Y-01-01'));

    -- 1. borrar histórico (rápido)
...
CREATE EVENT IF NOT EXISTS evt_rotar_particiones_viaje
ON SCHEDULE EVERY 1 YEAR
...
-- Rotacion de vehicluos

DELIMITER $$

CREATE EVENT IF NOT EXISTS evt_rotar_particiones_vehiculo
ON SCHEDULE EVERY 1 YEAR
STARTS '2027-01-01 02:10:00'
DO
BEGIN
    DECLARE v_hist BIGINT;
    DECLARE v_act  BIGINT;
...
-- Rotacion de telemetria

DELIMITER $$

CREATE EVENT IF NOT EXISTS evt_rotar_particiones_telemetria
ON SCHEDULE EVERY 1 YEAR
STARTS '2027-01-01 02:20:00'
DO
BEGIN
    DECLARE v_hist BIGINT;
    DECLARE v_act  BIGINT;
